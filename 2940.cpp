class Solution {
    vector<int> segTree;
    void buildSegmentTree(vector<int>& heights, int start, int end,
                          int st_idx) {
        if (start == end) {
        ...
        segTree[st_idx] =
            heights[segTree[2 * st_idx]] >= heights[segTree[2 * st_idx + 1]]
                ? segTree[2 * st_idx]
                : segTree[2 * st_idx + 1];
    }
    int rangeMaxQuery(vector<int>& heights, int qs, int qe, int start, int end,
                      int st_idx = 1) {
        ...
        int mid = start + (end - start) / 2,
            left_max = rangeMaxQuery(heights, qs, qe, start, mid, 2 * st_idx),
            right_max =
                rangeMaxQuery(heights, qs, qe, mid + 1, end, 2 * st_idx + 1);
        if (left_max == INT_MIN)
            return right_max;
...
public:
    vector<int> leftmostBuildingQueries(vector<int>& heights,
        ...
        for (auto query : queries) {
            ...
            int low = bob, high = n - 1, ans = INT_MAX;
            while (low <= high) {
                int mid = low + (high - low) / 2,
                    rmq = rangeMaxQuery(heights, low, mid, 0, n - 1, st_idx);
